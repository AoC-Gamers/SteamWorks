...
$steamworksSdkName = if ($env:STEAMWORKS_SDK_NAME) { $env:STEAMWORKS_SDK_NAME } else { "sdk" }
$steamworksSdkDir = if ($env:STEAMWORKS_SDK_DIR) { $env:STEAMWORKS_SDK_DIR } else { Join-Path $root $steamworksSdkName }
...
function Find-VcVarsAll {
  ...
  foreach ($rootDir in $searchRoots) {
    ...
    $vcvarsall = Get-ChildItem $rootDir -Recurse -Filter vcvarsall.bat -ErrorAction SilentlyContinue |
...
foreach ($requiredDir in @($hl2sdkDir, $sourcemodDir, $mmsourceDir, $steamworksSdkDir)) {
...
try {
  & $venvPython $configureScript `
    ...
    --steamworks-path $steamworksSdkDir `
    --enable-optimize `
    --target x86
...
$extBin = Get-ChildItem $packageDir -Filter "steamworks.ext*.dll" | Select-Object -First 1 -ExpandProperty FullName
if (-not $extBin) {
  throw "Build completed but no SteamWorks extension binary was found in $packageDir."
}

$canonicalExtBin = Join-Path $packageDir "steamworks.ext.dll"
